#!/usr/bin/env bun
// Acceptance fixture (JWB-326). One invocation = one real OS process acting as
// one role, making one guarded change to the shared durable lane store.
//
// The acceptance harness spawns this file as genuinely separate `bun` processes
// so that "orchestrator restart" and "supervisor restart" mean what they say: a
// new PID re-attaching to the same SQLite file, with no in-memory state carried
// over. This is the multi-process proof of durable, re-attachable lane state.
//
// Every command prints exactly one JSON envelope line (including this process's
// pid) to stdout and exits non-zero on any rejection.
//
...
//   bun lane-actor.ts drive    --db P --lane L --event validating --as orchestrator:alice
...
//   bun lane-actor.ts policy   --db P --key merge --value blocked --as orchestrator:alice
//   bun lane-actor.ts status   --db P --lane L
//   bun lane-actor.ts reconcile --db P --lane L --as supervisor:s1

import type { LaneAuthority, LaneEvent, LaneEventKind, LaneRole } from "../../src/lane.ts";
 ...
 * colons are separators, because a laneScope (e.g. `wf-lane:JWB-326`) may itself
 * contain colons. Authority derives from the credential, never the event body.
 */
...
const EVENT_KIND: Record<string, LaneEventKind> = {
  ...
  failed: "lane.failed",
  attention_required: "lane.attention_required",
  recovery_required: "lane.recovery_required",
};
  ...
  try {
    switch (command) {
      ...
      case "reconcile": {
        ...
        // active lane. It mutates only when the lane needs recovery; a healthy
