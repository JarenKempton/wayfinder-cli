...
function snapshot(value: unknown): Snapshot {
  ...
  if (
    ...
    (input.snapshotSequence as number) < 0 ||
  ...
  return { sequence: input.snapshotSequence as number, projects, threads };
}
...
export class T3Adapter {
  ...
  async #dispatch(
    ...
    try {
      ...
      if (!Number.isSafeInteger(result.sequence) || (result.sequence as number) < 0)
