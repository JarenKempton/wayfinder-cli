import type { FrontierTrackerAdapter } from "../../domain/contracts.ts";
...
import { type AssignmentState, AssignmentTrackerAdapter } from "./common.ts";
import {
  fetchTransport,
  type HttpTransport,
  object,
  ...
  implements FrontierTrackerAdapter
{
    ...
    return nodes.map((node, order) =>
      normalizeLinearTicket(
        ...
        ticketRefSchema.parse(
          `${parsed.adapter}:${parsed.instance}:${parsed.workspace}:ticket:${string(node.id, "issue id")}`,
        ),
        order,
      ),
    );
  ...
  async readAssignment(ticket: TicketRef): Promise<AssignmentState> {
    ...
    const assignee =
      issue.assignee === null
        ? undefined
        : actorRefSchema.parse(string(object(issue.assignee, "Linear assignee").id, "assignee id"));
    return {
...
function normalizeLinearTicket(
        ...
        return [
          {
            blocking: ticketRefSchema.parse(
              `${parsed.adapter}:${parsed.instance}:${parsed.workspace}:ticket:${string(blocker.id, "blocking issue id")}`,
            ),
            blocked: ref,
  ...
  const assignee =
    node.assignee === null || node.assignee === undefined
      ? undefined
      : actorRefSchema.parse(string(object(node.assignee, "Linear assignee").id, "assignee id"));
  const state = object(node.state, "Linear state");
  return {
    ref,
    ...(typeof node.title === "string" ? { title: node.title } : {}),
    ...(typeof node.description === "string" ? { description: node.description } : {}),
    map: mapRefSchema.parse(
      `${parsed.adapter}:${parsed.instance}:${parsed.workspace}:map:${mapId}`,
    ),
    kind,
