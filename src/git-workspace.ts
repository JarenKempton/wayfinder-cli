...
export class GitWorkspaceAdapter implements WorkspaceAdapter {
  ...
  constructor(repository: RepositorySpec, git: GitCommandExecutor = new BunGitCommandExecutor()) {
  ...
  #ticketId(ticket: TicketRef): string {
    ...
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(parsed.nativeId)) {
...
function canonicalRepository(repository: RepositorySpec): RepositorySpec {
  if (!repository.remote || !repository.baseBranch) {
    throw new Error("Repository remote and base branch are required");
  }
  return {
    ...repository,
...
export function parseWorktrees(output: string): WorktreeRecord[] {
  return output
    ...
    .flatMap((lines) => {
      ...
      return [{ path: resolve(path), ...(branch ? { branch } : {}) }];
    });
}
